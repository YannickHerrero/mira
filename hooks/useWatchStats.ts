...
import { watchProgressTable, mediaTable } from "@/db/schema";
...
export function useWatchStats() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      // 1. Fetch all watch progress rows
      ...
      // 2. Basic counts
      const moviesCompleted = completedRows.filter(
        (r) => r.mediaType === "movie"
      ).length;
      const episodesCompleted = completedRows.filter(
        (r) => r.mediaType === "tv"
      ).length;
      ...
      const totalWatchTimeSeconds = completedRows.reduce(
        (sum, r) => sum + (r.duration ?? 0),
        0
      );

      // 4. Collect unique (tmdbId, mediaType) pairs from completed rows
      ...
      for (const row of completedRows) {
        ...
        const rowMonth = row.watchedAt.slice(0, 7); // "YYYY-MM"
