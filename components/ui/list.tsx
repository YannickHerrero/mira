...
export const ListHeader: React.FC<ListHeaderProps> = ({ children, className, ...props }) => {
	return (
		<View className={cn("py-1.5 px-4 text-base", className)} {...props}>
			{children}
		</View>
	);
};
...
const List: React.FC<ListProps> = ({ children, className, ...props }) => {
	...
	const modifiedChildren = childrenArray.map((child, index) => {
		...
		if (isFirstChild || (isValidPrevSibling && prevSibling.type === ListHeader)) {
		...
		return React.cloneElement(element, {
			className: cn(element.props.className, injectClassName),
		});
	});
